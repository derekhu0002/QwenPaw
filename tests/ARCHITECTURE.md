...
- That same contract-test entrypoint now also freezes one still-open startup behavior gap from the updated intent handoff: a real runtime must register a lease heartbeat automatically before any user prompt mentions lease warmup or lease expiry. That startup-heartbeat contract is currently expected to fail until development closes the product background Heartbeat Emitter gap.

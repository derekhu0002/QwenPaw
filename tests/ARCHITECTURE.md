...
- That same contract-test entrypoint now also owns the new live reconnect recovery guard `contract/security/test_lease_recovery_semantics_contract.py::test_security_center_live_reconnect_gap_proof_restores_access`.
...
- Observation point: the restarted runtime must first materialize a local missing-gap proof for the same reconnect session, then automatically drive Security Center back to `ALIGNED` or `TRUSTED` with `gap_status=VALIDATED` or `CLEAR`, `recovery_gate_status=CLEAR`, `recovery_required=false`, refreshed durable lease timing, and no continued 423 recovery-gated blocking at the端侧 access boundary. Current repository state is expected to fail, and the currently observed failure is that the restarted session still does not produce the local gap-proof anchor set needed to drive the rest of recovery.

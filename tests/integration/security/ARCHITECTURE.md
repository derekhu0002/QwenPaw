...
- In the current repository state, the harness is still expected to fail with business-readable `Non_Repudiation_Gap`, `Audit_Integrity_Lockdown_Gap`, and `Prompt_Injection_Guard_Gap` categories where the live runtime lacks the required tool-boundary probes, durable ledger semantics, readable lock-mode surfacing, or durable rejected-event projection with Security_Rejection_Nonce semantics.

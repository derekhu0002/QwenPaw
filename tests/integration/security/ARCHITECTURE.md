...
- Freeze runtime-inspection expectations for `sec-e2e-024`, `sec-e2e-025`, and `sec-e2e-021`: tool-boundary context spying, direct physical ledger inspection, local hash-chain verification, checkpoint-loss-as-tamper handling with readable `UNTRUSTED` state, durable rejected-event evidence with trace-bound nonce semantics, and pre-execution evidence ordering.
...
- In the current repository state, the harness is expected to fail with business-readable `Non_Repudiation_Gap`, `Audit_Integrity_Lockdown_Gap`, and `Prompt_Injection_Guard_Gap` categories because the live runtime still lacks the required tool-boundary probes, durable ledger semantics, readable lock-mode surfacing, recovery handshake evidence, and durable rejected-event projection with Security_Rejection_Nonce semantics.

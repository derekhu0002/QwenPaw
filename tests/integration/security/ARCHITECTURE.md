...
- Keep testcase bodies business-readable and GIVEN/WHEN/THEN-shaped by routing repository inspection through local harness abstractions instead of raw HTTP, raw environment mutation, or other plumbing in the testcase body.
...
- path: harness.py
  ...
  role: business-readable harness abstraction that inspects repository evidence, performs runtime-shape spying, reads the physical audit ledger seam, and reports architecture-stage security gaps with business categories
...
- testcase_name: sec-e2e-024-end-to-end-non-repudiation-evidence-chain
  ...
  control_point: authenticate an employee, request a delegated high-risk action, and provide the required confirmation through the harness
  observation_point: the resulting observations prove or disprove tool-boundary implicit context propagation, a complete user -> agent -> plugin -> tool evidence chain, a physical USER_CONFIRMATION audit record on disk, local hash-chain integrity, and the order 'store evidence before releasing the high-risk tool'
...
- In the current repository state, the harness is expected to fail with a business-readable `Non_Repudiation_Gap` category because the runtime still lacks a context spy at the high-risk tool boundary, a physical confirmation record with digest and prior-hash, a hash-chain verifier, and an enforced pre-execution evidence write.

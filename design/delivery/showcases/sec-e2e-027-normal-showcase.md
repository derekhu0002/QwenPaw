...
一个完全不知道 QwenPaw 安全审计能力的普通用户，像平时一样打开 QwenPaw 继续处理工作；QwenPaw runtime 启动后已经在后台自动注册首个租约心跳，并在设备保持在线时持续发送心跳，Security Center 在后台确认租约有效，用户不会看到额外的安全术语、恢复流程或人工干预。
...
- QwenPaw runtime 启动后会自动向 Security Center 注册首个 lease heartbeat；演示中不需要、也不应该让普通用户输入任何包含 lease heartbeat monitoring 或 lease window expires 的提示词来触发租约注册。
...
- 这些租约字段来自 runtime 启动后的后台 Heartbeat Emitter，而不是来自某条用户输入的 warmup prompt。
- `recovery_required` 为 `false`。
- 没有出现恢复闸门、异常告警或模型访问拒绝。

## 控制点与观察点

- 控制点：QwenPaw runtime 启动后自动注册首个租约心跳，普通用户在设备在线且租约未过期时连续发起两次普通模型请求。
...
- Security Center 能在用户不输入任何 warmup prompt 的情况下看到租约时间字段，说明启动路径和正常在线路径都在持续维护 active-defense 基线。
...
- Security Center 只有在用户输入特定 lease warmup 提示词后才出现租约字段。
- 客户端在租约未过期时被错误标记为 `UNTRUSTED`。

## 讲解口径

“这个场景先不讲攻击，也不讲审计术语。用户只是正常打开 QwenPaw 工作，系统应该像普通 AI 助理一样流畅响应。QwenPaw 启动后会在后台自动注册租约心跳，并持续维护设备在线租约和可信状态，但不会打扰用户。只有当设备真的长期失联或连续性无法证明时，用户才会看到恢复提示。正常情况下，它应该是安静的、可解释的、但不抢戏的。”
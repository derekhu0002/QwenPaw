...
- Delivery is reopened on the cloud-side lease-persistence seam: current repository evidence does not yet prove that the runtime's canonical heartbeat payload carries `lease_ttl_seconds` end-to-end into durable Security Center state. Overview and timeline may still synthesize nonzero lease timing while the durable store keeps `last_heartbeat_at=0` and `lease_expires_at=0`, which prevents a stopped runtime from being downgraded through the true TTL path.

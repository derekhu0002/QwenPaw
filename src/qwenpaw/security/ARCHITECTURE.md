...
- Delivery is now reopened on the live reconnect recovery seam: the supporting guard at `../../tests/contract/security/test_lease_recovery_semantics_contract.py::test_security_center_live_reconnect_gap_proof_restores_access` now executes against a reset-state API/Web/runtime stack and currently fails. Control point: establish one trusted canonical client, let it downgrade to `UNTRUSTED` with `divergence_reason=lease_ttl_expired`, restart the same runtime, then attempt restored model access without manual `/security-center/v1/recovery/handshake` proof injection. Observation point: the runtime must first materialize a locally constructible full missing-gap proof for the restarted session, then automatically post equivalent recovery evidence, refresh the same canonical client's lease heartbeat, close the recovery gate, and stop returning recovery-gated blocking. Current repository state still fails at that first reconnect step because the restarted session does not yet produce the local gap-proof anchor set.

...
- The live integration runtime still does not show a tool-boundary runtime spy, a direct disk-level USER_CONFIRMATION record, a local hash-chain verifier, a readable `UNTRUSTED` state after checkpoint loss, a pre-upload recovery handshake, a durable rejection event with non-static trace-bound Security_Rejection_Nonce semantics, a hash-break curve chart with explicit fork point, or explicit backend-API and operator-web projections that remain physically separate from the edge runtime.

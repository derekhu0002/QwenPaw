...
- The reopened sec-e2e-027 client-identity gap is not closed. The new live supporting guard at `../../tests/contract/security/test_lease_recovery_semantics_contract.py::test_security_center_projects_one_online_runtime_as_one_canonical_terminal` proves that one online runtime is still projected as two canonical Security Center terminals after startup heartbeat plus later session-scoped security traffic.
- Coding/Repair must converge startup heartbeat, recovery preflight, lockdown projection, restored-access projection, and Security Center timeline reads onto one repository-owned canonical runtime client id while keeping browser or session ids only as audit metadata or display aliases.
- Coding/Repair must also prevent a live non-forked runtime from being projected as `DIVERGED` with `recovery_gate_status=OPEN` on a mere bootstrap-shadow mismatch once the real audit-chain head advances.

...
- Delivery is now reopened on the startup normal-admission seam: current repository evidence does not yet prove that a clean canonical startup heartbeat is admitted as a normal online terminal. A fresh runtime can still be projected as `GAP_VALIDATION_REQUIRED` with `gap_status=REQUIRED`, `recovery_gate_status=OPEN`, and `divergence_reason=missing_gap_proof` before any offline lease-expiry event has occurred.

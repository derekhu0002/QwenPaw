...
- Current repository evidence now closes the reopened sec-e2e-027 client-identity gap: heartbeat emission, recovery preflight, lockdown projection, restored-access projection, and Security Center timeline reads all converge on one canonical runtime client id, while browser or session ids remain audit metadata rather than separate cloud terminal identities.

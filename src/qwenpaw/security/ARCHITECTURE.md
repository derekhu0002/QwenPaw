...
- The updated intent handoff exposes one remaining product-control gap above those repaired semantics: lease registration still begins through prompt-classified warmup in `../app/routers/console.py`, and runtime startup does not yet emit an initial heartbeat automatically through a background Heartbeat Emitter.

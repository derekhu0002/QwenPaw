...
- Current repository evidence still leaves one sec-e2e-027 architecture gap open: startup heartbeat currently uses `runtime-heartbeat::<fingerprint>` while later recovery, lockdown, and restored-access flows continue to address Security Center by raw session id, so one live runtime can appear as multiple cloud clients and the bootstrap-only heartbeat client can be misclassified as `DIVERGED` without a real local fork.

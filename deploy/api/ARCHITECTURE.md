...
- The critical architecture guard at `../../tests/architecture/security-runtime-client-identity-boundary.test.js` now passes as a regression guard: the backend keeps one canonical runtime client id per live edge runtime while allowing session aliases only as display-level metadata, so one online runtime no longer surfaces as a false `DIVERGED` client.

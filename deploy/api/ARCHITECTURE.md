...
- A newer supporting contract in that same file still fails by design: runtime startup does not yet register any lease client before user prompt traffic begins, so the product background Heartbeat Emitter requirement from the updated intent handoff remains open even though deploy/api is ready to project lease timing once heartbeats arrive.

...
- A new supporting guard at `../../tests/contract/security/test_lease_recovery_semantics_contract.py::test_security_center_live_reconnect_gap_proof_restores_access` is now physically mounted as the live reconnect recovery delivery gate and is expected to fail in the current repository state. Control point: after the canonical client downgrades to `UNTRUSTED` with `divergence_reason=lease_ttl_expired`, restart the same runtime and attempt restored model access without manual `/security-center/v1/recovery/handshake` proof injection. Observation point: deploy/api must observe the same canonical client return to `ALIGNED` or `TRUSTED` with `gap_status=VALIDATED` or `CLEAR`, `recovery_gate_status=CLEAR`, `recovery_required=false`, and refreshed durable lease timing. Current failure signal is earlier in that chain: the restarted runtime still does not materialize a locally constructible reconnect gap proof for the same session, so the backend cannot yet observe an automatic proof post from the real runtime path.

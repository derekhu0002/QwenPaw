...
- A critical architecture guard now fails by design for the current repository state: `../../tests/architecture/security-runtime-client-identity-boundary.test.js` proves the edge runtime still splits startup heartbeat client identity from later recovery and projection identity, so the backend can surface one online runtime as a false `DIVERGED` client.

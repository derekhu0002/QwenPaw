...
- The new live supporting guard at `../../tests/contract/security/test_lease_recovery_semantics_contract.py::test_security_center_projects_one_online_runtime_as_one_canonical_terminal` currently fails and reopens canonical-runtime-client-id-003: after startup heartbeat plus one session-scoped security flow, deploy/api still reports `client_count=2` with separate `runtime-heartbeat::<fingerprint>` and `runtime-session::<workspace>::<session>` canonical ids for one live runtime.
- Coding/Repair must collapse those two records into one canonical terminal, preserve session ids only as metadata or display aliases, and ensure bootstrap alignment cannot later project a live non-forked runtime as `DIVERGED` with `recovery_gate_status=OPEN` on `local_hash_mismatch` alone.

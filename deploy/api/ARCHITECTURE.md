...
- Current repository evidence still proves the backend can project lease timing through operator query surfaces and keeps recovery gated until cloud-side gap validation succeeds, but the latest intent handoff reopens sec-e2e-027 because that read model can still hide a durable-store gap: overview and timeline may show nonzero lease timing while `security-center-store.json` still persists `last_heartbeat_at=0` and `lease_expires_at=0` for the same canonical runtime client.
...
- The new supporting guard at `../../tests/contract/security/test_lease_recovery_semantics_contract.py::test_security_center_persists_runtime_lease_fields_and_downgrades_after_stop` is now frozen as the reopened sec-e2e-027 delivery gate. Control point: start API, web, and runtime; observe a projected lease-bearing canonical client; inspect the durable Security Center store; stop the runtime; then wait for TTL expiry. Observation point: the same canonical client must already persist nonzero `last_heartbeat_at`, `lease_ttl_seconds`, and `lease_expires_at` in `security-center-store.json` before runtime stop, and overview/timeline must later expose `UNTRUSTED` with `divergence_reason=lease_ttl_expired`. Current repository state is expected to fail this guard until Coding/Repair persists lease timing instead of relying on read-model projection.
- The new critical architecture guard at `../../tests/architecture/security-runtime-lease-persistence-boundary.test.js` freezes the API/store seam that must carry `lease_ttl_seconds` through `deploy/api/app.py` into the durable lease registry. Coding/Repair must remove projection-only fallback as the acceptance mechanism and make the durable store the source of truth for TTL expiry.
